...
class Solution2021Day24(Solution):

    def solve(self, inputs: Inputs) -> None:
        self.run(inputs.input.splitlines())
    ...
    def run(self, program: list[str]) -> None:
        ...
        self.result_1 = "".join(str(min(max(x, 1), 9)) for x in max_value)
        self.result_2 = "".join(str(min(max(x, 1), 9)) for x in min_value)
